...
class RegimePerfAnalytics:
    ...
    def analyze(self, trades_df: pd.DataFrame, regime_history: pd.DataFrame) -> pd.DataFrame:
        ...
        # Handle regime_history with timestamp as column (from RegimeHistoryStore.save_csv)
        if "timestamp" in regime_history.columns:
            regime_history = regime_history.set_index("timestamp")
