...
- Next: (1) default `use_historical_universe: true` for measurement campaigns going forward, (2) re-run the Discovery gauntlet on universe-aware substrate, (3) populate the missing CSVs via `scripts/fetch_universe.py`, (4) re-evaluate which alpha generators survive in 2023 vs. the others.
...
### [HIGH] No purged k-fold / embargo / CPCV anywhere in the validation chain — explicit class-of-validation gap (2026-05-07)
- Engine: D (`engine_d_discovery/wfo.py`, `engine_d_discovery/discovery.py` gates 2-8)
- First flagged: 2026-05-07 by external-reviewer audit (engine-auditor lens)
- Status: not started
- Description: A repo-wide grep for `purged`, `embargo`, `CPCV`, `combinatorial.*purged` returns ZERO hits across `engines/`, `core/`, and `scripts/`. The WFO at `engines/engine_d_discovery/wfo.py:98` uses gap-less train→test concatenation (`test_start = full_timeline[train_end_idx]`, comment: "gapless"). With features whose horizons span 60-252 days (`core/feature_foundry/features/realized_vol_60d.py:36` reads 61 trailing closes; `beta_252d` reads 252 closes), and with autocorrelated daily returns, a gapless WFO leaks training-window state into the first ~60 test bars and inflates OOS Sharpe systematically. Gate 2 (PBO) bootstraps an attribution stream (`engine_d_discovery/robustness.py::bootstrap_returns_stream`) but the underlying stream was generated without an embargo — the bootstrap doesn't fix the upstream leak. Gate 8 (DSR multiple-testing) defaults `n_trials_for_dsr=1` (`engine_d_discovery/discovery.py:708`), so multiple-testing correction is OFF by default; Gate 8 trivially passes (line 788: `"gate_8_passed": True,   # Default True = SKIPPED`). Combined: every Sharpe number that contributed to the 6-self-falsified-findings sequence was measured under a no-embargo, no-multiple-testing-correction substrate.
- Why this is HIGH: this is the single most-likely meta-cause of the "exciting findings keep dying" pattern. Without embargo, the WFO produces optimistic OOS Sharpes that recover the training-window edge; without DSR-with-real-n, repeated GA trials get free passes. The substrate-aware (universe-aware) collapse of 1.296 → 0.507 already revealed one substrate flaw; the embargo gap is structurally identical (different substrate flaw, same class).
- Recommended next step: implement embargo (≥ max-feature-horizon trading days, e.g. 252 for the longest active feature) at `wfo.py:98` between train_end and test_start. Default `n_trials_for_dsr` to the actual number of candidates the GA + tree scanner generated this cycle (counted at the orchestration level), not 1. Add a static-analysis check at `core/observability/leakage_detector.py` that, given a feature's declared `horizon` in its `@feature` model card, enforces `embargo_days ≥ horizon` at validation time.

### [HIGH] No experiment tracker (MLflow / W&B level) — every measurement campaign is named-anchor-on-disk; comparing 6 weeks of Sharpe headlines requires manual cross-referencing of memory files (2026-05-07)
- Engine: cross-cutting infra (none — that's the issue)
- First flagged: 2026-05-07 by external-reviewer audit
- Status: not started
- Description: Repo-wide grep for `MLflow`, `mlflow`, `wandb`, `experiment.tracker` returns ZERO hits. Measurement state lives in (a) named anchors via `scripts/run_isolated.py --save-anchor`, (b) per-run trade logs at `data/trade_logs/<run_id>/`, (c) memory files like `project_universe_aware_collapses_2026_05_09.md`, (d) audit docs under `docs/Measurements/<YYYY-MM>/`. There is no single queryable surface where every (config_hash, universe_substrate, governor_anchor_md5, n_trials, Sharpe, PSR, DSR) tuple is stored. When 6 findings get falsified in 6 weeks, the team's only way to compare them is to hand-walk the memory files. The `decision_diary` (`core/observability/decision_diary.py`, 249 LOC) is a per-run audit log, not an experiment tracker — it doesn't index across runs.
- Why this is HIGH: external-review preparation requires reproducible cross-run comparisons. Right now the team cannot answer "show me every same-config rerun across the substrate-honest universe with DSR > 0.95 and PSR > 0.95" without grepping memory files. This is the infrastructure layer that would catch substrate drift one finding earlier.
- Recommended next step: add a thin SQLite-backed run registry in `core/observability/run_registry.py`. Every `ModeController.run_backtest` and `validate_candidate` call writes a row keyed by (run_id, config_hash, universe_hash, governor_anchor_md5) with all metrics from `MetricsEngine.calculate_all` + `n_trials` + `embargo_days` + commit_sha. Cockpit dashboard_v2 gets a `runs_tab.py` that surfaces it. This is propose-first (cross-cutting infra) — flag for user decision.

### [MEDIUM] Cockpit dashboard_v2 surfaces no regime-conditional Sharpe, no tail metrics, no PSR/DSR — performance_tab is point-in-time PnL only (2026-05-07)
- Engine: cockpit/dashboard_v2 (consumes Engine F's `regime_performance` + `core/metrics_engine` outputs)
- First flagged: 2026-05-07 by external-reviewer audit
- Status: not started
- Description: `grep -rn "regime_performance\|RegimePerfAnalytics\|regime_conditional"` in `cockpit/dashboard_v2/` returns ZERO hits. `grep -n "tail\|kurtosis\|conditional.sharpe\|skew"` in `performance_tab.py`, `dashboard_tab.py`, `analytics_tab.py` returns one comment match. `core/metrics_engine.py` already exposes PSR, DSR, Tail Ratio, Skew, Excess Kurtosis (lines 53, 158, 196 — confirmed); `engine_f_governance/regime_analytics.py::RegimePerfAnalytics` already produces per-edge per-regime Sharpe; neither is read by the cockpit. The dashboard is therefore optimized for the metric (point-in-time PnL) least diagnostic of "is this strategy actually working?"
- Why this is MEDIUM (not HIGH): this is a presentation gap on top of correctly computed metrics, not a correctness bug. But it's the surface the operator stares at, and it omits exactly the metrics that would have flagged the substrate-bias issue earlier (regime-conditional Sharpe collapse in 2024 vs 2023).
- Recommended next step: extend `cockpit/dashboard_v2/tabs/performance_tab.py` and the analytics parent tab to render PSR / DSR / Tail Ratio / Skew / Kurtosis from `core/measurement_reporter.py` output, and a regime-conditional Sharpe heatmap (4 regime states × edge_id) from `data/governor/regime_edge_performance.json`. No new computation needed — both data sources exist.

### [MEDIUM] TierClassifier lives in Engine A package but is invoked exclusively by Engine F to mutate edges.yml (charter-package mismatch) (2026-05-07)
- Engine: A + F (boundary)
- First flagged: 2026-05-07 by external-reviewer audit
- Status: not started
- Description: `engines/engine_a_alpha/tier_classifier.py:83` defines `TierClassifier`. The only production caller is `engines/engine_f_governance/governor.py:627` (`from engines.engine_a_alpha.tier_classifier import TierClassifier`) inside `evaluate_tiers`, which writes `tier` and `combination_role` back to `edges.yml` (line 638-645, `write=True`). Charter says: A is the Researcher (forecasts only), F owns lifecycle and weight assignments. A module that classifies an edge's *role* (alpha/feature/context) and writes the result to the edge registry is a lifecycle decision dressed as a forecast helper, and it is in the wrong package. The F11 known smell (write-back to edges.yml during a run) compounds with this — the writer is in F, but the writer's logic is imported from A.
- Why this is MEDIUM: the functional outcome is correct (tier reclassification happens, audit-trailed); the charter line is fuzzy. But the import direction violates "F can be completely disabled without breaking A" (charter Engine F invariant 5) — if F is disabled, tier_classifier's only invocation site disappears and the module becomes dead code in A's package.
- Recommended next step: move `tier_classifier.py` to `engines/engine_f_governance/tier_classifier.py` (it's a Governance lifecycle tool that consumes A's output, not an A internal). Update the one import site at `governor.py:627`. Add a unit test asserting A's `engine_a_alpha/__init__.py` does not transitively import TierClassifier, so disabling F genuinely leaves A intact.

### [MEDIUM] run_isolated.py snapshots 4 governor files + 6 module globals but does NOT snapshot pandas / numpy / random global state, and does not assert PYTHONHASHSEED on subprocess calls (2026-05-07)
- Engine: orchestration / determinism harness
- First flagged: 2026-05-07 by external-reviewer audit
- Status: not started
- Description: `scripts/run_isolated.py:75-80` lists `ISOLATED_FILES` (edges.yml, edge_weights.json, regime_edge_performance.json, lifecycle_history.csv) and lines 107-121 list `ISOLATED_GLOBALS` (5 module-level caches). The harness re-execs to set `PYTHONHASHSEED=0` (line 67-69) — good. But it does NOT: (a) call `random.seed()` / `np.random.seed()` / `pd.options.mode.copy_on_write` reset between runs; (b) snapshot the runtime pandas option state (some edges set `pd.options.mode.chained_assignment = None` at import time); (c) check `numpy.show_config()` BLAS backend (OpenBLAS-vs-MKL produces different floating-point sums for >1e6 elements); (d) propagate `PYTHONHASHSEED=0` to subprocesses if the harness ever shells out (it doesn't today, but this is a regression-trap as soon as a child process is added). The 3-rep bitwise-identical canon md5 is real today, but every test of the harness has been on the same single Python interpreter with the same module-import order. A worktree on a different machine, or a CI runner with a different BLAS, could produce different bytes for the same anchor.
- Why this is MEDIUM: today the harness works (verified). The risk is a future regression that surfaces only in CI / on a new machine, after which the team will spend weeks bisecting before realizing the harness never claimed to control these axes.
- Recommended next step: extend the isolated() context manager to also (1) `random.seed(0); np.random.seed(0)` on entry, (2) `os.environ["OMP_NUM_THREADS"] = "1"` (BLAS determinism), (3) record `numpy.__config__.blas_info` and pandas options into the canon trail, (4) emit a one-time WARNING if any of those don't match the anchor's recorded values. The harness should be defensive about more than just governor file state.
