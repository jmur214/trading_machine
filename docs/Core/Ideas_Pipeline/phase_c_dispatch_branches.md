...
The structural review that the pre-commit calls for = engine completion. The user surfaced that the engines aren't operating per their charters; the substrate-honest 0.507 result was generated by a system where:
- Engine C's portfolio-allocation logic (`compute_target_allocations`) is never called in the backtest loop
- HRPOptimizer + TurnoverPenalty live in Engine A's `signal_processor.py:228-242` (charter inversion F4 from audit)
- No portfolio-level diversification, correlation-aware sizing, factor-exposure caps
- Engine D's Discovery cycle has produced 0 promoted edges across the project's history
- Engine E's HMM is empirically coincident (memory `project_regime_signal_falsified_2026_05_06`)
