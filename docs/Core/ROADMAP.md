...
- [ ] **Phase ε (step 2): Demote & re-validate remaining 12 base edges.** NOW UNBLOCKED — Phase α v2 ships. Demote hand-entered actives to `candidate`, run Phase γ validation on each via `EvolutionController`. Expected: some edges (like momentum_edge_v1 with Sharpe +2.2 in isolation) auto-promote back to active; weak ones either stay candidate or go to failed. Each validation ~30min. 1 day for cycle + observation.
- [ ] **Phase α v3 (optional refinement):** Revival gate currently uses `pnls[-20:]` which approximates "post-pause trades." For precision, read `lifecycle_history.csv` to find pause timestamp and filter pnls to post-pause only. Approximation works for high-frequency edges; this refinement matters more for edges that trade sparsely.
- [ ] **Phase δ: GA fitness function upgrade.** `0.5*OOS_Sharpe_vs_benchmark + 0.3*(1-PBO) + 0.2*OOS/IS_ratio`. Failure-reason tracking for credit assignment. Seed from validated winners, not random. 1 week.
- [ ] **Phase ζ: New alpha templates.** PEAD, cross-sectional momentum (sector-neutral), low-vol factor, event-driven. Only after α+ε prove autonomy works. 2-3 weeks.

**Acceptance**: after α+ε, at least one currently-active edge auto-retires on evidence; combined in-sample Sharpe >1.2; OOS beats SPY on ≥2 of 3 walk-forward splits.
