...
3) Updates governor edge weights (via engines.engine_f_governance.governor if available; otherwise local merge)
4) Persists:
...
CLI:
    python -m engines.engine_f_governance.system_governor --once
    python -m engines.engine_f_governance.system_governor --watch --interval 60
...
try:
    from engines.engine_f_governance import governor as _GOVERNOR_MOD  # type: ignore
except Exception:
    _GOVERNOR_MOD = None
...
class SystemGovernor:
    ...
    def _update_edge_weights(self, metrics_obj: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if _GOVERNOR_MOD and hasattr(_GOVERNOR_MOD, "update_weights"):
            try:
                logger.info("Updating weights via engines.engine_f_governance.governor.update_weights(...)")
                # Common forms:
                try:
